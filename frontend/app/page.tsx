              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="bg-[#EA4335]/5 border border-[#EA4335]/10 p-4 rounded-xl space-y-3">
                  <span className="bg-[#EA4335]/10 text-[#D93025] border border-[#EA4335]/25 text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded inline-flex items-center gap-1">
                    <AlertTriangle className="h-3.5 w-3.5 text-[#D93025]" /> Strict Compliance Bans
                  </span>
...
export default function Home() {
  // Tabs
  const [activeTab, setActiveTab] = useState<'optimizer' | 'auditor' | 'analytics'>('optimizer');
  ...
  const [trendsInterval, setTrendsInterval] = useState<'weekly' | 'monthly' | 'quarterly'>('monthly');
  const [trendsMetric, setTrendsMetric] = useState<'budget' | 'engagement' | 'score' | 'creators'>('budget');
  ...
  const fetchAiInsights = async () => {
    ...
    try {
      ...
      console.error(err);
      setAiInsightsError("Could not retrieve AI CMO insights. Ensure backend server is connected.");
    } finally {
  ...
  const fetchAnalyticsData = async () => {
    ...
    try {
      const trendsRes = await fetch(`${API_BASE}/analytics/trends?interval=${trendsInterval}`);
      ...
      console.error(err);
      setAnalyticsError("Failed to connect to backend server. Make sure FastAPI server is running on port 8000.");
    } finally {
  ...
  const handleDeleteCampaign = async (id: number) => {
    if (!confirm("Are you sure you want to delete this campaign snapshot from history?")) return;
    try {
                ...
                <div className="flex flex-col gap-0.5">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge className="bg-[#FBBC05]/10 border border-[#FBBC05]/20 text-[#B78103] text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full animate-pulse-glow inline-flex items-center gap-1">
                      <Sparkles className="h-2.5 w-2.5 text-[#B78103]" /> Budget Optimized
                    </Badge>
                  </div>
                                ...
                                <div className="flex flex-wrap gap-1.5 max-w-[80%]">
                                  ...
                                  {influencer.ResponseTime !== undefined && influencer.ResponseTime > 0 && (
                                    <Badge className="bg-muted text-muted-foreground border-none text-[8px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded-full inline-flex items-center gap-1">
                                      ...
                                      <span>{influencer.ResponseTime}h response</span>
                                    </Badge>
                                  )}
                                </div>
                                    ...
                                    {(influencer.ai_review || liveReviews[influencer.Name]) && (
                                      <div className="bg-[#F8F9FA] p-3 rounded-xl border border-border/50">
                                        <div className="flex justify-between items-center mb-1.5">
                                          ...
                                          >
                                            {liveReviewsLoading[influencer.Name] ? (
                                              "Generating..."
                                            ) : (
                                              <>
        ...
        <TabsContent value="analytics" className="flex flex-col gap-6 flex-1">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Historical roster intelligence and spend optimization metrics synced via SQLite local database.
              </p>
            </div>
            <Button
          ...
          {analyticsError && (
            <Alert variant="destructive" className="border-red-200 bg-red-50 text-[#EA4335] p-3.5 rounded-xl flex items-start gap-3">
              <AlertTriangle className="h-5 w-5 shrink-0 mt-0.5" />
              <div>
                <AlertTitle className="font-bold text-xs">Analytics Sync Failed</AlertTitle>
                <AlertDescription className="text-xs mt-0.5">{analyticsError}</AlertDescription>
              </div>
            </Alert>
          )}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Card className="bg-white border border-border p-4 rounded-2xl relative overflow-hidden shadow-sm/5">
              ...
              <p className="text-[9px] font-bold uppercase tracking-wider text-muted-foreground">Cumulative Spend</p>
              <h3 className="text-xl font-black text-[#4285F4] mt-1.5">
                ₹{campaignHistory.reduce((sum, c) => sum + (c.budget_used || 0), 0).toLocaleString('en-IN')}
              </h3>
              <span className="text-[9px] text-muted-foreground block mt-0.5">₹15 Lakhs limit per campaign</span>
            </Card>

            <Card className="bg-white border border-border p-4 rounded-2xl relative overflow-hidden shadow-sm/5">
              <div className="absolute top-0 left-0 w-full h-[3px] bg-[#34A853]" />
              <p className="text-[9px] font-bold uppercase tracking-wider text-muted-foreground">Average Engagement</p>
              <h3 className="text-xl font-black text-[#34A853] mt-1.5">
                {campaignHistory.length 
                  ? (campaignHistory.reduce((sum, c) => sum + (c.avg_engagement_rate || 0), 0) / campaignHistory.length).toFixed(2)
                  : "0.00"}%
              </h3>
              <span className="text-[9px] text-muted-foreground block mt-0.5">Campaign baseline target</span>
            </Card>

            <Card className="bg-white border border-border p-4 rounded-2xl relative overflow-hidden shadow-sm/5">
              <div className="absolute top-0 left-0 w-full h-[3px] bg-[#FBBC05]" />
              <p className="text-[9px] font-bold uppercase tracking-wider text-muted-foreground">Quality Score Avg</p>
              <h3 className="text-xl font-black text-[#B78103] mt-1.5">
                {campaignHistory.length
                  ? (campaignHistory.reduce((sum, c) => sum + (c.avg_score || 0), 0) / campaignHistory.length).toFixed(1)
                  : "0.0"}
              </h3>
              <span className="text-[9px] text-muted-foreground block mt-0.5">Scale of 0-100 composite</span>
            </Card>

            <Card className="bg-white border border-border p-4 rounded-2xl relative overflow-hidden shadow-sm/5">
              <div className="absolute top-0 left-0 w-full h-[3px] bg-[#EA4335]" />
              <p className="text-[9px] font-bold uppercase tracking-wider text-muted-foreground">Shortlist Run History</p>
              <h3 className="text-xl font-black text-[#EA4335] mt-1.5">{campaignHistory.length} Campaigns</h3>
              <span className="text-[9px] text-muted-foreground block mt-0.5">SQLite database records</span>
            </Card>
          </div>
            ...
            <div className="relative h-[320px] w-full flex items-center justify-center">
              {analyticsLoading && (
                <div className="absolute inset-0 bg-white/70 backdrop-blur-xs z-10 flex flex-col items-center justify-center gap-2">
                  ...
                  <p className="text-xs font-bold text-[#4285F4]">Compiling local trends...</p>
                </div>
              )}
              ...
              {trendsData.length === 0 ? (
                <div className="flex flex-col items-center text-center justify-center p-8 border border-border border-dashed rounded-2xl w-full h-full">
                  ...
                  <h4 className="font-bold text-sm text-foreground/80">No historical trends compiled</h4>
                  <p className="text-xs text-muted-foreground mt-0.5">Upload campaigns or trigger optimizer snapshots to populate SQLite records.</p>
                </div>
              ) : (
                (() => {
                  const width = 800;
                  ...
                  const values = trendsData.map(d => {
                    ...
                    if (trendsMetric === 'engagement') return d.avg_engagement_rate || 0;
                  ...
                  const coords = trendsData.map((d, i) => {
                    ...
                    const val = trendsMetric === 'budget' ? d.budget_used : trendsMetric === 'engagement' ? d.avg_engagement_rate : trendsMetric === 'score' ? d.avg_score : d.total_shortlisted;
                    const y = height - paddingY - (val / maxVal) * chartHeight;
                    return { x, y, val, period: d.period };
                  });

                  const pathD = coords.map((c, i) => `${i === 0 ? 'M' : 'L'} ${c.x} ${c.y}`).join(' ');
                  const fillD = coords.length ? `${pathD} L ${coords[coords.length - 1].x} ${height - paddingY} L ${coords[0].x} ${height - paddingY} Z` : '';
                      ...
                      >
                        <defs>
                          <linearGradient id="chartGradient" x1="0" y1="0" x2="0" y2="1">
                            <stop offset="0%" stopColor={trendsMetric === 'budget' ? '#4285F4' : trendsMetric === 'engagement' ? '#34A853' : trendsMetric === 'score' ? '#FBBC05' : '#EA4335'} stopOpacity="0.25" />
                            <stop offset="100%" stopColor={trendsMetric === 'budget' ? '#4285F4' : trendsMetric === 'engagement' ? '#34A853' : trendsMetric === 'score' ? '#FBBC05' : '#EA4335'} stopOpacity="0.00" />
                          </linearGradient>
                          <filter id="lineGlow" x="-20%" y="-20%" width="140%" height="140%">
                            <feDropShadow dx="0" dy="5" stdDeviation="6" floodColor={trendsMetric === 'budget' ? '#4285F4' : trendsMetric === 'engagement' ? '#34A853' : trendsMetric === 'score' ? '#FBBC05' : '#EA4335'} floodOpacity="0.18" />
                          </filter>
                        </defs>
                        ...
                        {/* X Axis Labels */}
                        {trendsData.map((d, i) => {
                          const x = paddingX + (trendsData.length > 1 ? (i / (trendsData.length - 1)) * chartWidth : chartWidth / 2);
                          return (
                            <text
                              key={i}
                              x={x}
                        ...
                        <path
                          d={pathD}
                          fill="none"
                          stroke={trendsMetric === 'budget' ? '#4285F4' : trendsMetric === 'engagement' ? '#34A853' : trendsMetric === 'score' ? '#B78103' : '#EA4335'}
                          strokeWidth="3.5"
                          strokeLinecap="round"
                          strokeLinejoin="round"
                          filter="url(#lineGlow)"
                        ...
                        {/* Coordinate Dots */}
                        {coords.map((c, i) => (
                          <g key={i} className="group/dot cursor-pointer" onClick={() => setSelectedPeriodDetail(trendsData[i])}>
                            {selectedPeriodDetail?.period === c.period && (
                              <circle
                                ...
                                stroke={trendsMetric === 'budget' ? '#4285F4' : trendsMetric === 'engagement' ? '#34A853' : trendsMetric === 'score' ? '#FBBC05' : '#EA4335'}
                              />
                            )}
                            {selectedPeriodDetail?.period === c.period && (
                              <circle
                                ...
                                r="8.5"
                                className={`${
                                  trendsMetric === 'budget' ? 'fill-[#4285F4]/20 stroke-[#4285F4]' : trendsMetric === 'engagement' ? 'fill-[#34A853]/20 stroke-[#34A853]' : trendsMetric === 'score' ? 'fill-[#FBBC05]/20 stroke-[#FBBC05]' : 'fill-[#EA4335]/20 stroke-[#EA4335]'
                                } stroke-[2]`}
                              />
                            )}
                            <circle
                              ...
                              className={`opacity-0 group-hover/dot:opacity-100 transition duration-150 ${
                                trendsMetric === 'budget' ? 'fill-[#4285F4]/10' : trendsMetric === 'engagement' ? 'fill-[#34A853]/10' : trendsMetric === 'score' ? 'fill-[#FBBC05]/10' : 'fill-[#EA4335]/10'
                              }`}
                            />
                            <circle
                              cx={c.x}
                              ...
                              className={`fill-white stroke-[3.5] transition duration-150 ${
                                trendsMetric === 'budget' ? 'stroke-[#4285F4]' : trendsMetric === 'engagement' ? 'stroke-[#34A853]' : trendsMetric === 'score' ? 'stroke-[#B78103]' : 'stroke-[#EA4335]'
                              }`}
                            />
                            ...
                            <g className="opacity-0 group-hover/dot:opacity-100 pointer-events-none transition-all duration-200 translate-y-1 group-hover/dot:translate-y-0">
                              ...
                              <text
                                ...
                                className={`text-[10px] font-black ${
                                  trendsMetric === 'budget' ? 'fill-[#4285F4]' : trendsMetric === 'engagement' ? 'fill-[#34A853]' : trendsMetric === 'score' ? 'fill-[#B78103]' : 'fill-[#EA4335]'
                                }`}
                              >
                                {trendsMetric === 'budget'
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 flex flex-col gap-6">
              ...
              {selectedPeriodDetail && (
                <Card className="bg-white border border-border p-5 rounded-2xl relative overflow-hidden shadow-sm/5 transition duration-300 hover:border-black/[0.08]">
                  ...
                  <div className={`absolute top-0 left-0 w-1 h-full ${
                    trendsMetric === 'budget' ? 'bg-[#4285F4]' : trendsMetric === 'engagement' ? 'bg-[#34A853]' : trendsMetric === 'score' ? 'bg-[#FBBC05]' : 'bg-[#EA4335]'
                  }`} />
                  
                  <div className="flex justify-between items-start">
                    <div>
                      <span className="text-[9px] text-muted-foreground font-black uppercase tracking-widest block">Interactive Period Drilldown</span>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-4">
                    <div className="bg-[#F8F9FA] p-3 rounded-xl border border-border/40">
                      <p className="text-[8px] font-bold text-muted-foreground uppercase tracking-wider">Avg Spend Allocations</p>
                      <p className="text-sm font-black text-[#4285F4] mt-1">₹{selectedPeriodDetail.budget_used?.toLocaleString('en-IN')}</p>
                    </div>
                    <div className="bg-[#F8F9FA] p-3 rounded-xl border border-border/40">
                      <p className="text-[8px] font-bold text-muted-foreground uppercase tracking-wider">Avg Engagement Rate</p>
                      <p className="text-sm font-black text-[#34A853] mt-1">{selectedPeriodDetail.avg_engagement_rate}%</p>
                    </div>
                    <div className="bg-[#F8F9FA] p-3 rounded-xl border border-border/40">
                      <p className="text-[8px] font-bold text-muted-foreground uppercase tracking-wider">Avg Quality Score</p>
                      <p className="text-sm font-black text-[#B78103] mt-1">{selectedPeriodDetail.avg_score}</p>
                    </div>
                    <div className="bg-[#F8F9FA] p-3 rounded-xl border border-border/40">
                      <p className="text-[8px] font-bold text-muted-foreground uppercase tracking-wider">Campaign Snapshot Count</p>
                      <p className="text-sm font-black text-[#EA4335] mt-1">{selectedPeriodDetail.campaigns_count} snap runs</p>
                    </div>
                  </div>
                  
                  <p className="text-[9px] text-muted-foreground mt-3 italic text-right">
                    💡 Click on any coordinate dot on the SVG line chart above to focus another period dynamically.
                  </p>
                </Card>
              )}
              ...
              {selectedCampaignIds.length === 2 && (() => {
                const compA = campaignHistory.find(c => c.id === selectedCampaignIds[0]);
                const compB = campaignHistory.find(c => c.id === selectedCampaignIds[1]);
                ...
                return (
                  <Card className="bg-gradient-to-tr from-[#4285F4]/5 via-[#34A853]/2 to-transparent border border-[#4285F4]/15 p-5 rounded-2xl shadow-md animate-fade-in relative overflow-hidden">
                    ...
                    <div className="grid grid-cols-3 gap-3 border-collapse text-xs">
                      {/* Headers */}
                      <div className="font-extrabold text-muted-foreground py-2">Parameter</div>
                      <div className="bg-white/80 p-2.5 rounded-xl border border-border/40 font-black text-center truncate text-[#4285F4]">{compA.campaign_name}</div>
                      <div className="bg-white/80 p-2.5 rounded-xl border border-border/40 font-black text-center truncate text-[#34A853]">{compB.campaign_name}</div>
                      
                      {/* Date Row */}
                      <div className="font-bold text-muted-foreground py-2 border-t border-border/40">Upload Date</div>
                      <div className="p-2 text-center border-t border-border/40 font-semibold">{compA.uploaded_at.substring(0, 10)}</div>
                      <div className="p-2 text-center border-t border-border/40 font-semibold">{compB.uploaded_at.substring(0, 10)}</div>
                      
                      {/* Shortlisted Row */}
                      <div className="font-bold text-muted-foreground py-2 border-t border-border/40">Shortlist Size</div>
                      <div className="p-2 text-center border-t border-border/40 font-black text-[#4285F4]">{compA.total_shortlisted} creators</div>
                      <div className="p-2 text-center border-t border-border/40 font-black text-[#34A853]">{compB.total_shortlisted} creators</div>
                      
                      {/* Spend Row */}
                      <div className="font-bold text-muted-foreground py-2 border-t border-border/40">Total Spend</div>
                      <div className={`p-2 text-center border-t border-border/40 font-black rounded-lg ${compA.budget_used < compB.budget_used ? 'bg-[#34A853]/10 text-[#34A853]' : ''}`}>
                        ₹{compA.budget_used.toLocaleString('en-IN')}
                      </div>
                      <div className={`p-2 text-center border-t border-border/40 font-black rounded-lg ${compB.budget_used < compA.budget_used ? 'bg-[#34A853]/10 text-[#34A853]' : ''}`}>
                        ₹{compB.budget_used.toLocaleString('en-IN')}
                      </div>
                      
                      {/* ER Row */}
                      <div className="font-bold text-muted-foreground py-2 border-t border-border/40">Avg Engagement</div>
                      <div className={`p-2 text-center border-t border-border/40 font-black rounded-lg ${compA.avg_engagement_rate > compB.avg_engagement_rate ? 'bg-[#34A853]/10 text-[#34A853]' : ''}`}>
                        {compA.avg_engagement_rate}%
                      </div>
                      <div className={`p-2 text-center border-t border-border/40 font-black rounded-lg ${compB.avg_engagement_rate > compA.avg_engagement_rate ? 'bg-[#34A853]/10 text-[#34A853]' : ''}`}>
                        {compB.avg_engagement_rate}%
                      </div>
                      
                      {/* Score Row */}
                      <div className="font-bold text-muted-foreground py-2 border-t border-border/40">Avg Creator Quality</div>
                      <div className={`p-2 text-center border-t border-border/40 font-black rounded-lg ${compA.avg_score > compB.avg_score ? 'bg-[#34A853]/10 text-[#34A853]' : ''}`}>
                        {compA.avg_score}
                      </div>
                      <div className={`p-2 text-center border-t border-border/40 font-black rounded-lg ${compB.avg_score > compA.avg_score ? 'bg-[#34A853]/10 text-[#34A853]' : ''}`}>
                        {compB.avg_score}
                      </div>
                      
                      {/* Tier Row */}
                      <div className="font-bold text-muted-foreground py-2 border-t border-border/40">Tier 2/3 Representation</div>
                      <div className={`p-2 text-center border-t border-border/40 font-black rounded-lg ${compA.tier23_percentage >= 40 ? 'text-[#34A853]' : 'text-[#EA4335]'}`}>
                        {compA.tier23_percentage}% {compA.tier23_percentage >= 40 ? '✓' : '✗'}
                      </div>
                      <div className={`p-2 text-center border-t border-border/40 font-black rounded-lg ${compB.tier23_percentage >= 40 ? 'text-[#34A853]' : 'text-[#EA4335]'}`}>
                        {compB.tier23_percentage}% {compB.tier23_percentage >= 40 ? '✓' : '✗'}
                      </div>
                    </div>
              ...
              <div className="flex flex-col gap-4">
                ...
                {campaignHistory.length === 0 ? (
                  <Card className="bg-white border border-border border-dashed p-10 rounded-2xl text-center flex flex-col items-center">
                    <FileSpreadsheet className="h-8 w-8 text-muted-foreground/60 mb-2" />
                    <h4 className="text-sm font-bold text-foreground">No historical snapshots saved</h4>
                    <p className="text-xs text-muted-foreground mt-0.5">Database history list is currently clear.</p>
                  </Card>
                ) : (
                      ...
                      <table className="w-full text-left text-xs border-collapse">
                        ...
                        <tbody>
                          {campaignHistory.map((c: any, index: number) => {
                            ...
                            return (
                              <tr key={index} className="border-b border-border/50 hover:bg-muted/30 transition">
                                    ...
                                    onChange={(e) => {
                                      if (e.target.checked) {
                                        if (selectedCampaignIds.length >= 2) {
                                          alert("You can select up to 2 campaigns to compare at the same time.");
                                          return;
                                        }
                                ...
                                <td className="p-3.5 font-bold text-foreground max-w-[180px] truncate">{c.campaign_name}</td>
                                <td className="p-3.5 text-muted-foreground font-semibold">
                                  {c.uploaded_at.substring(0, 10)} {c.uploaded_at.substring(11, 16)}
                                </td>
                                <td className="p-3.5 text-center font-bold text-[#4285F4]">{c.total_shortlisted}</td>
                                <td className="p-3.5 text-center font-extrabold text-[#34A853]">₹{c.budget_used?.toLocaleString('en-IN')}</td>
                                <td className="p-3.5 text-center font-bold text-[#34A853]">{c.avg_engagement_rate}%</td>
                                <td className="p-3.5 text-center">
            ...
            <div className="lg:col-span-1 flex flex-col gap-6">
              <Card className="bg-white border border-border p-5 rounded-2xl relative overflow-hidden shadow-sm/5 flex flex-col gap-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <span className="text-[8px] text-gray-500 uppercase tracking-widest font-black block">Analytics Executive Copilot</span>
                    <h3 className="text-sm font-black text-slate-900 tracking-tight mt-0.5">CMO Strategic AI Insights</h3>
                  </div>
                </div>
                
                <p className="text-xs text-muted-foreground leading-relaxed">
                  Run a real-time, dynamic marketing audit using standard OpenAI models to diagnose budget flows, anomalies, and outcome forecasts based on SQLite history.
                </p>
                
                {aiInsightsError && (
                  <Alert variant="destructive" className="border-red-200 bg-red-50 text-[#EA4335] p-3 rounded-xl flex items-start gap-2.5">
                    <AlertTriangle className="h-4 w-4 shrink-0 mt-0.5" />
                    <span className="text-[10px] leading-snug">{aiInsightsError}</span>
                  </Alert>
                )}
                ...
                {aiInsights && (
                  <Card className="bg-[#F8F9FA] p-4 rounded-xl border border-border shadow-inner text-left max-h-[480px] overflow-hidden flex flex-col gap-3 animate-fade-in">
                    <div className="flex justify-between items-center pb-2 border-b border-border/60 shrink-0">
                      <span className="text-[9px] font-black text-[#4285F4] uppercase tracking-wider">Dynamic Report</span>
                      <Button
                        onClick={() => {
